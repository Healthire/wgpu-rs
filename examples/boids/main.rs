...
impl framework::Example for Example {
    ...
    fn init(
        ...
        let boids_module = device.create_shader_module(include_glsl!("examples/boids/boids.comp"));

        let vs_module = device.create_shader_module(include_glsl!("examples/boids/shader.vert"));
        let fs_module = device.create_shader_module(include_glsl!("examples/boids/shader.frag"));
        ...
        seed.copy_from_slice((0 as u128).as_bytes());
        let mut rng = rand::rngs::SmallRng::from_seed(seed);
        for particle_instance_chunk in initial_particle_data.chunks_mut(4) {
            particle_instance_chunk[0] = 2.0 * (rng.gen_range(-0.5, 0.5)); // posx
            particle_instance_chunk[1] = 2.0 * (rng.gen_range(-0.5, 0.5)); // posy
            particle_instance_chunk[2] = 2.0 * (rng.gen_range(-0.5, 0.5)) * 0.1; // velx
            particle_instance_chunk[3] = 2.0 * (rng.gen_range(-0.5, 0.5)) * 0.1;
            // vely
        }
